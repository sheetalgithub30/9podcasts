...
type podcast struct {
	...
	Website_address string `json:"website_address`
...
func createPodcast(c echo.Context) (err error) {
	...
	err = db.QueryRow(q, pd.Title, pd.Description, pd.Website_address, pd.Category_id, pd.Language, pd.IsExplicit, pd.Cover_art_path, pd.Author_name, pd.Author_email, pd.Created_at, pd.Updated_at, pd.Published_at).Scan(&pd.Id)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func getPodcast(c echo.Context) (err error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&pd.Id, &pd.Title, &pd.Description, &pd.Website_address, &pd.Category_id, &pd.Language, &pd.IsExplicit, &pd.Cover_art_path, &pd.Author_name, &pd.Author_email, &pd.Created_at, &pd.Updated_at, &pd.Published_at)
		if err != nil {
			fmt.Println(err)
		}
